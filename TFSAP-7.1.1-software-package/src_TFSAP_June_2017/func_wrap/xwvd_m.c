...
void mexFunction( int nlhs, MATRIX *plhs[], int nrhs, NEED_CONST MATRIX *prhs[])
{
  ...
  double *signal1_r, *signal1_i;
  double *signal2_r, *signal2_i;
  ...
  int num_slices, window_order, window_r2;
   ...
   if( nrhs < 4 ) {
      tfsaErr( "xwvd", "Not enough input arguments" );
      winNTcheck( nlhs, plhs );
      return;
   }

   if( nrhs > 5 ) {
      tfsaErr( "xwvd", "Too many input arguments" );
      winNTcheck( nlhs, plhs );
      return;
   }

   if( nlhs > 1 ) {
      tfsaErr( "xwvd", "Too many output arguments" );
      winNTcheck( nlhs, plhs );
      return;
   }
   ...
   if( !mxIsNumeric( prhs[0] ) || !MXFULL( prhs[0] ) ) {
      tfsaErr( "xwvd", "Input must be a vector" );
      winNTcheck( nlhs, plhs );
      return;
   }
   ...
   if( (M==1 && N==1) || (M!=1 && N!=1) ) {
      tfsaErr( "xwvd", "Input1 must be a vector" );
      winNTcheck( nlhs, plhs );
      return;
   }
   ...
   if( !mxIsNumeric( prhs[1] ) || !MXFULL( prhs[1] ) ) {
      tfsaErr( "xwvd", "Input2 must be a vector" );
      winNTcheck( nlhs, plhs );
      return;
   }
   ...
   if ((M>N?M:N) != (int)signal_length) {
      tfsaErr( "xwvd", "Input signals must be the same length");
      winNTcheck( nlhs, plhs );
      return;
   }
   ...
   if( !GoodScalar( (MATRIX *)prhs[2] ) ) {
      tfsaErr( "xwvd", "Smoothing window length must be a scalar" );
      winNTcheck( nlhs, plhs );
      return;
   }
   else
   ...
   if( window_length < 1 ) {
      tfsaErr( "xwvd", "Window length must be greater than zero" );
      winNTcheck( nlhs, plhs );
      return;
   }
  ...
  if( window_length > signal_length )  {
    tfsaWarning( "xwvd", "Window length has been truncated to signal length" );
    window_length = (int)signal_length - 1;
  }

  if( (window_length%2) == 0) {
     tfsaErr("xwvd", "Window length must be odd");
     winNTcheck( nlhs, plhs );
    return;
  }
   ...
   if( !GoodScalar( (MATRIX *)prhs[3] ) ) {
      tfsaErr( "xwvd", "Time resolution must be a scalar" );
      winNTcheck( nlhs, plhs );
      return;
   }
   else
     time_res = (int)*(mxGetPr( prhs[3]));

   /* Parameter value check */
   if( time_res < 1 ) {
      tfsaErr( "xwvd", "Time resolution must be greater than zero" );
      winNTcheck( nlhs, plhs );
      return;
   }

   if( time_res > signal_length ) {
      tfsaErr( "xwvd", "Time resolution must be no greater than half the signal length" );
      winNTcheck( nlhs, plhs );
      return;
   }
   ...
   if (nrhs==5) {
	 ...
	 tfsaErr( "xwvd", "FFT length must be a scalar" );
	 winNTcheck( nlhs, plhs );
	 return;
      }
      else
  ...
  if( fft_length < 1 ) {
    tfsaErr( "xwvd", "FFT length must be greater than zero" );
    winNTcheck( nlhs, plhs );
    return;
  }
   ...
   signal1_r = mxGetPr( prhs[0]);
   if (mxIsComplex(prhs[0]))
     signal1_i = mxGetPi( prhs[0]);
   else
     signal1_i = NULL;

   signal2_r = mxGetPr( prhs[1]);
   if (mxIsComplex(prhs[1]))
     signal2_i = mxGetPi( prhs[1]);
   else
     signal2_i = NULL;

   num_slices = (int)signal_length/time_res - 1;
  ...
  xwvd( signal1_r, signal1_i, signal2_r, signal2_i, signal_length, result_r, result_i, num_slices,
