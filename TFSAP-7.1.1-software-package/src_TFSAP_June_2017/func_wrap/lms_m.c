...
void mexFunction( int nlhs, MATRIX *plhs[], int nrhs, NEED_CONST MATRIX *prhs[])
{
   ...
   switch( nrhs ) {
    ...
    case( 1 ):	 /* One input */
      ...
      tfsaErr("lms", "Not enough input arguments" );
    ...
    case( 2 ):	 /* Two inputs */
	 ...
	 tfsaErr( "lms", "Input must be a vector" );
	 winNTcheck( nlhs, plhs );
	 return;
      }
	...
	signal_i = NULL;
	 ...
	 tfsaErr( "lms", "Forgetting factor 'mu' must be a real scalar" );
	 winNTcheck( nlhs, plhs );
	 return;
      }

      mu = (double)*(mxGetPr( prhs[1] ) );    /* Get value */

      if( mu < 0.0 ) {
	 tfsaErr( "lms", "Forgetting factor 'mu' must be a non-negative scalar"	);
	 winNTcheck( nlhs, plhs );
	 return;
      }
      break;
  ...
  default:   /* More than two inputs */
      ...
      tfsaErr( "lms", "Too many input arguments" );
      winNTcheck( nlhs, plhs );
      return;
      break;
   ...
   switch( nlhs ) {
    ...
    case( 1 ):
      ...
      if( plhs[0] == NULL ) {
	 tfsaErr( "lms", "Internal memory allocation failure" );
	 winNTcheck( nlhs, plhs );
	 return;
      }
    ...
    default:  /* more than one output */
      ...
      tfsaErr( "lms", "Too many output arguments" );
      winNTcheck( nlhs, plhs );
      return;
      break;
   }
  ...
  /* Least Mean Square Adaptive Estimator */

  lms( signal_r, signal_i, signal_length, result, mu );
