...
void mexFunction( int nlhs, MATRIX *plhs[], int nrhs, NEED_CONST MATRIX *prhs[])
{
  ...
  int nplts, window_order, window_r2;
   ...
   if( nrhs < 3 ) {
      tfsaErr( "sfpe", "Not enough input arguments" );
      winNTcheck( nlhs, plhs );
      return;
   }

   if( nrhs > 4 ) {
      tfsaErr( "sfpe", "Too many input arguments" );
      winNTcheck( nlhs, plhs );
      return;
   }

   if( nlhs > 1 ) {
      tfsaErr( "sfpe", "Too many output arguments" );
      winNTcheck( nlhs, plhs );
      return;
   }
   ...
   if( !mxIsNumeric( prhs[0] ) || !MXFULL( prhs[0] ) ) {
      tfsaErr( "sfpe", "Input must be a vector" );
      winNTcheck( nlhs, plhs );
      return;
   }
  ...
  if( !GoodScalar( (MATRIX *)prhs[1] ) ) {
    tfsaErr( "sfpe", "Smoothing window length must be a scalar" );
    winNTcheck( nlhs, plhs );
      return;
   }
   else
   ...
   if( window_length < 1 ) {
      tfsaErr( "sfpe", "Window length must be greater than zero" );
      winNTcheck( nlhs, plhs );
      return;
   }
   ...
   if( window_length > signal_length ) {
      tfsaWarning( "sfpe", "Window length has been truncated to signal length" );
   ...
   if( !GoodScalar( (MATRIX *)prhs[2] ) ) {
      tfsaErr( "sfpe", "Time resolution must be a scalar" );
      winNTcheck( nlhs, plhs );
      return;
   }
   else
   ...
   /* Parameter value check */
  if( time_res < 1 ) {
    tfsaErr( "sfpe", "Time resolution must be greater than zero" );
    winNTcheck( nlhs, plhs );
    return;
  }

  if (time_res > signal_length) {
      tfsaErr( "sfpe", "Time resolution must be no greater than signal length" );
      winNTcheck( nlhs, plhs );
      return;
   }
   ...
   if (nrhs==4) {
     if( !GoodScalar( (MATRIX *)prhs[3] ) ) {
       tfsaErr( "sfpe", "FFT length must be a scalar" );
       winNTcheck( nlhs, plhs );
       return;
     }
     else
  ...
  if( fft_length < 1 ) {
    tfsaErr( "sfpe", "FFT length must be greater than zero" );
    winNTcheck( nlhs, plhs );
    return;
  }
  ...
  sfpe( signal_r, signal_i, signal_length, result, nplts,
