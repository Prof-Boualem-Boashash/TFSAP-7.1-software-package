...
figure
subplot(2,2,1)
plot(t,sig);axis([0 8 min(sig) max(sig)])
title('Original signal')

subplot(2,2,2)
plot(t1,signal_dim);axis([0 8 min(signal_dim) max(signal_dim)])
title('Level 1 of the DWT decomposition')

subplot(2,2,3)
plot(t2,signal_dim2);axis([0 8 min(signal_dim2) max(signal_dim2)])
title('Level 2 of the DWT decomposition')

subplot(2,2,4)
plot(t3,signal_dim3);axis([0 8 min(signal_dim3) max(signal_dim3)])
